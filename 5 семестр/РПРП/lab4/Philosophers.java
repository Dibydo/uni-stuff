                ...
                switch (state) {
                    case 0:
                        if (!(leftFork.mutex.isHeldByCurrentThread()) && leftFork.isFree()) {
                            leftFork.grab();
                            System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number + 1) + " берёт левую вилку");
                        }
                    case 1:
                        if (!(rightFork.mutex.isHeldByCurrentThread()) && rightFork.isFree()) {
                            rightFork.grab();
                            System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number + 1) + " берёт правую вилку");
                        }
                    case 2:
                        if (leftFork.mutex.isHeldByCurrentThread() && rightFork.mutex.isHeldByCurrentThread()) {
                            int sleepTime = (int)(Math.random() * 3000);
                            System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number+1) + " ест в течение " + sleepTime +"мс");
                            try {
                    ...
                    case 3:
                        if (leftFork.mutex.isHeldByCurrentThread()) {
                            leftFork.release();
                            System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number + 1) + " кладёт левую вилку");
                        }
                        if (rightFork.mutex.isHeldByCurrentThread()) {
                            rightFork.release();
                            System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number + 1) + " кладёт правую вилку");
                        }
                        break;
                    case 4:
                        int thinkTime = (int)(Math.random() * 2000);
                        System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философ " + (number+1) + " размышляет в течение " + thinkTime +"мс");
                        try {
        ...
        while (elapsedTime < 60000) {
            try {
                ...
                if (deadlock) {
                    ...
                    System.out.println(System.nanoTime() / 10_000_000 + ":	" + "Философы кладут вилки на стол");
                }
            }
            catch (Exception e) {
                e.printStackTrace(System.out);

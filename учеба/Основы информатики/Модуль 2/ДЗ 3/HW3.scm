...
(define (cut-first xs x) ; -top
  (if (null? x)
      xs
      (cut-first (append xs (cons (car x) '())) (cdr x))))
...
(define (derivative a) ; само взятие поизводной (derivative)
  (cond
    ...
    ((sum? a) (sum-build (derivative (cadr a)) (derivative (caddr a))))

    ((prod? a) (sum-build (prod-build (derivative (cadr a)) (caddr a)) (prod-build (cadr a) (derivative (caddr a)))))

    ((difference? a) (difference-build (derivative (cadr a)) (derivative (caddr a))))

    ((sin? a) (prod-build (derivative (cadr a)) (list 'cos (cadr a))))

    ((cos? a) (prod-build (derivative (cadr a)) (list '* -1 (list 'sin (cadr a)))))

    ((expt? a) (prod-build (derivative (cadr a)) (list '* (caddr a) (list 'expt (cadr a) (- (caddr a) 1)))))

    ((expot? a) (prod-build (derivative (caddr a)) (list '* a (list 'ln (cadr a)))))

    ((exp? a) (prod-build (derivative (cadr a)) a))

    ((ln? a) (prod-build (derivative (cadr a)) (list '/ 1 (cadr a))))

    ((multiprod? a) (sum-build (prod-build (derivative (cadr a)) (cut-first (list '*) (cdr (cdr a)))) (prod-build (cadr a) (derivative (cut-first (list '*) (cdr (cdr a)))))))
